import { useEffect, useState } from "react";
  ...
  /* ── Constants (fill after Sepolia deployment) ──────────────────────── */
  const FACTORY_V2  = "";
  const IMPL_V2     = "";
  const TX_DEPLOY   = "";
  ...
  /* ── Tiny helpers ────────────────────────────────────────────────────── */
  const short = (v: string, head = 8, tail = 6) =>
      v ? `${v.slice(0, head)}...${v.slice(-tail)}` : "Pending";

  function CopySpan({ value, display }: { value: string; display?: string }) {
      const [ok, setOk] = useState(false);
      if (!value) return (
          <span style={{ fontFamily: "'JetBrains Mono','Courier New',monospace", fontSize: 12, color: "rgba(255,255,255,0.3)", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 6, padding: "3px 9px", fontStyle: "italic" }}>
              Pending deployment
          </span>
      );
      return (
  ...
  function AddrRow({ label, value, verified }: { label: string; value: string; verified?: boolean }) {
      return (
          <div style={{ padding: "12px 0", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 6 }}>
                  ...
                  {verified && value && <span style={{ fontFamily: "'Inter',sans-serif", fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: PASS, background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.22)", borderRadius: 4, padding: "1px 6px" }}>VERIFIED</span>}
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8 }}>
                  <CopySpan value={value} display={value ? short(value, 12, 6) : undefined} />
                  {value && <ExtLink href={`${ETHERSCAN}/address/${value}#code`}>Etherscan</ExtLink>}
              </div>
          </div>
      );
  }
  ...
  function TxRow({ label, hash }: { label: string; hash: string }) {
      return (
          <div style={{ padding: "12px 0", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 8 }}>
                  <CopySpan value={hash} display={hash ? short(hash, 12, 6) : undefined} />
                  {hash && <ExtLink href={`${ETHERSCAN}/tx/${hash}`}>Etherscan</ExtLink>}
              </div>
          </div>
      );
  }
          ...
          <div style={{ minHeight: "100vh", background: "#000000", color: "#fff" }}>
              ...
              <div style={{ position: "relative", overflow: "hidden" }}>
                  <div style={{ position: "absolute", inset: 0, zIndex: 0 }}>
                      ...
                      />
                      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom, rgba(0,0,0,0.1) 0%, rgba(0,0,0,0.6) 70%, #000 100%)" }} />
                  </div>
                  ...
                  <div style={{ position: "relative", zIndex: 1, maxWidth: W, margin: "0 auto", padding: pad }}>
                      <div style={{ padding: isMobile ? "100px 0 40px" : "110px 0 56px" }}>
                          <div>
                              ...
                              <p style={{ fontFamily: "'Inter',sans-serif", fontSize: isMobile ? 14 : 16, color: "rgba(255,255,255,0.5)", lineHeight: 1.65, margin: "0 0 32px", maxWidth: 640 }}>
                                  {sr.heroBody}
                              </p>
                              ...
                              <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                                  {[
                                      ...
                                      { val: "Ownable", label: sr.statLabels[1], color: "#EF4444" },
                                      { val: "No Pausable", label: sr.statLabels[2], color: PASS },
                                      { val: "MIT", label: sr.statLabels[3], color: "#06B6D4" },
                                  ].map(s => (
                                      <div key={s.label} style={{ background: "rgba(255,255,255,0.05)", border: `1px solid ${s.color}28`, borderRadius: 12, padding: "12px 18px", textAlign: "center", minWidth: 110 }}>
                                          <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 18, fontWeight: 800, letterSpacing: "-0.03em", color: s.color }}>{s.val}</div>
              ...
              <div style={{ maxWidth: W, margin: "0 auto", padding: pad }}>
                  ...
                  <div style={{ ...card({ padding: isMobile ? "28px 18px" : "36px 40px", marginBottom: 20, borderColor: `${PASS}22` }) }}>
                      ...
                      <div style={{ display: isMobile ? "block" : "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                          {sr.introItems.map((item, i) => (
                              <div key={i} style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${i < 1 ? "rgba(239,68,68,0.12)" : "rgba(34,197,94,0.1)"}`, borderRadius: 14, padding: "18px 20px", marginBottom: isMobile ? 12 : 0 }}>
                                  <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 13, fontWeight: 700, color: i < 1 ? "#EF4444" : PASS, marginBottom: 8, letterSpacing: "-0.01em" }}>{item.label}</div>
                  ...
                  <div style={{ ...card({ padding: isMobile ? "28px 18px" : "36px 40px", marginBottom: 20 }) }}>
                      ...
                      <AddrRow label={sr.addrLabels.factory} value={FACTORY_V2} verified={!!FACTORY_V2} />
                      <AddrRow label={sr.addrLabels.impl} value={IMPL_V2} verified={!!IMPL_V2} />
                  ...
                  <div style={{ ...card({ padding: isMobile ? "22px 18px" : "26px 32px", marginBottom: 60, borderColor: "rgba(255,255,255,0.06)" }), display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
                      <a
                          ...
                          style={{ display: "inline-flex", alignItems: "center", gap: 8, fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: 13, color: "rgba(255,255,255,0.5)", textDecoration: "none", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "10px 20px" }}
                      ...
                      <div>
                          <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(255,255,255,0.28)", marginBottom: 4, textAlign: "right" }}>
                              Next Record
                          </div>
                          <div style={{ fontFamily: "'Inter',sans-serif", fontSize: 15, fontWeight: 700, color: "rgba(255,255,255,0.7)", textAlign: "right" }}>
                              QryptSafe V3: Admin keys removed
                          </div>
                      </div>
                      <a
